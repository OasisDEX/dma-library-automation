import { Network } from '@deploy-configurations/types/network'
import { getActionHash } from '@deploy-configurations/utils/action-hash'
import { getAddressesFor } from '@dma-common/utils/common/addresses'
import { utils } from 'ethers'
import { task } from 'hardhat/config'

import { loadContractNames } from '../../../deploy-configurations/constants/load-contract-names'

task('get-hashes', 'get Addresses hashes').setAction(async (_: any, hre) => {
  const { name: network } = hre.network
  console.log(`Network: ${network}. Verifying contracts...\n`)
  const SERVICE_REGISTRY_NAMES = loadContractNames(network as Network)
  const {
    OPERATION_EXECUTOR,
    OPERATION_STORAGE,
    OPERATIONS_REGISTRY,
    PULL_TOKEN_ACTION,
    SEND_TOKEN_ACTION,
    SEND_TOKEN_AUTO_ACTION,
    SET_APPROVAL_ACTION,
    SWAP_ACTION,
    TAKE_FLASHLOAN_ACTION,
    UNWRAP_ETH_ACTION,
    WRAP_ETH_ACTION,
    RETURN_FUNDS_ACTION,
    AAVE_BORROW_ACTION,
    AAVE_DEPOSIT_ACTION,
    AAVE_WITHDRAW_ACTION,
    AAVE_PAYBACK_ACTION,
    POSITION_CREATED_ACTION,
  } = await getAddressesFor(network as Network)

  const opExecutorHash = utils.keccak256(
    utils.toUtf8Bytes(SERVICE_REGISTRY_NAMES.common.OPERATION_EXECUTOR),
  )
  console.log(
    'OPERATION_EXECUTOR',
    OPERATION_EXECUTOR,
    SERVICE_REGISTRY_NAMES.common.OPERATION_EXECUTOR,
    opExecutorHash,
  )

  const opStorageHash = utils.keccak256(
    utils.toUtf8Bytes(SERVICE_REGISTRY_NAMES.common.OPERATION_STORAGE),
  )
  console.log(
    'OPERATION_STORAGE',
    OPERATION_STORAGE,
    SERVICE_REGISTRY_NAMES.common.OPERATION_STORAGE,
    opStorageHash,
  )

  const opRegistryHash = utils.keccak256(
    utils.toUtf8Bytes(SERVICE_REGISTRY_NAMES.common.OPERATIONS_REGISTRY),
  )
  console.log(
    'OPERATIONS_REGISTRY',
    OPERATIONS_REGISTRY,
    SERVICE_REGISTRY_NAMES.common.OPERATIONS_REGISTRY,
    opRegistryHash,
  )

  const pullTokenHash = utils.keccak256(utils.toUtf8Bytes(SERVICE_REGISTRY_NAMES.common.PULL_TOKEN))
  console.log(
    'PULL_TOKEN_ACTION',
    PULL_TOKEN_ACTION,
    SERVICE_REGISTRY_NAMES.common.PULL_TOKEN,
    pullTokenHash,
  )

  const sendTokenHash = utils.keccak256(utils.toUtf8Bytes(SERVICE_REGISTRY_NAMES.common.SEND_TOKEN))
  console.log(
    'SEND_TOKEN_ACTION',
    SEND_TOKEN_ACTION,
    SERVICE_REGISTRY_NAMES.common.SEND_TOKEN,
    sendTokenHash,
  )
  const sendTokenAutoHash = utils.keccak256(
    utils.toUtf8Bytes(SERVICE_REGISTRY_NAMES.common.SEND_TOKEN_AUTO),
  )
  getActionHash(SERVICE_REGISTRY_NAMES.common.SEND_TOKEN_AUTO)
  console.log(
    'SEND_TOKEN_AUTO',
    SEND_TOKEN_AUTO_ACTION,
    SERVICE_REGISTRY_NAMES.common.SEND_TOKEN_AUTO,
    sendTokenAutoHash,
    getActionHash(SERVICE_REGISTRY_NAMES.common.SEND_TOKEN_AUTO),
    `\n`,
  )
  const setApprovalHash = utils.keccak256(
    utils.toUtf8Bytes(SERVICE_REGISTRY_NAMES.common.SET_APPROVAL),
  )
  console.log(
    'SET_APPROVAL_ACTION',
    SET_APPROVAL_ACTION,
    SERVICE_REGISTRY_NAMES.common.SET_APPROVAL,
    setApprovalHash,
  )

  const swapAction = utils.keccak256(utils.toUtf8Bytes(SERVICE_REGISTRY_NAMES.common.SWAP_ACTION))
  console.log('SWAP_ACTION', SWAP_ACTION, SERVICE_REGISTRY_NAMES.common.SWAP_ACTION, swapAction)

  const takeFlashloanAction = utils.keccak256(
    utils.toUtf8Bytes(SERVICE_REGISTRY_NAMES.common.TAKE_A_FLASHLOAN),
  )
  console.log(
    'TAKE_FLASHLOAN_ACTION',
    TAKE_FLASHLOAN_ACTION,
    SERVICE_REGISTRY_NAMES.common.TAKE_A_FLASHLOAN,
    takeFlashloanAction,
  )

  const positionCreatedAction = utils.keccak256(
    utils.toUtf8Bytes(SERVICE_REGISTRY_NAMES.common.POSITION_CREATED),
  )
  console.log(
    'POSITION_CREATED',
    POSITION_CREATED_ACTION,
    SERVICE_REGISTRY_NAMES.common.POSITION_CREATED,
    positionCreatedAction,
  )

  const unwrapAction = utils.keccak256(utils.toUtf8Bytes(SERVICE_REGISTRY_NAMES.common.UNWRAP_ETH))
  console.log(
    'UNWRAP_ETH_ACTION',
    UNWRAP_ETH_ACTION,
    SERVICE_REGISTRY_NAMES.common.UNWRAP_ETH,
    unwrapAction,
  )

  const wrapAction = utils.keccak256(utils.toUtf8Bytes(SERVICE_REGISTRY_NAMES.common.WRAP_ETH))
  console.log(
    'WRAP_ETH_ACTION',
    WRAP_ETH_ACTION,
    SERVICE_REGISTRY_NAMES.common.WRAP_ETH,
    wrapAction,
  )

  const returnFundsAction = utils.keccak256(
    utils.toUtf8Bytes(SERVICE_REGISTRY_NAMES.common.RETURN_FUNDS),
  )
  console.log(
    'RETURN_FUNDS_ACTION',
    RETURN_FUNDS_ACTION,
    SERVICE_REGISTRY_NAMES.common.RETURN_FUNDS,
    returnFundsAction,
  )

  const aaveBorrowAction = utils.keccak256(utils.toUtf8Bytes(SERVICE_REGISTRY_NAMES.aave.v2.BORROW))
  console.log(
    'AAVE_BORROW_ACTION',
    AAVE_BORROW_ACTION,
    SERVICE_REGISTRY_NAMES.aave.v2.BORROW,
    aaveBorrowAction,
  )

  const aaveDepositAction = utils.keccak256(
    utils.toUtf8Bytes(SERVICE_REGISTRY_NAMES.aave.v2.DEPOSIT),
  )
  console.log(
    'AAVE_DEPOSIT_ACTION',
    AAVE_DEPOSIT_ACTION,
    SERVICE_REGISTRY_NAMES.aave.v2.DEPOSIT,
    aaveDepositAction,
  )

  const aaveWithdrawAction = utils.keccak256(
    utils.toUtf8Bytes(SERVICE_REGISTRY_NAMES.aave.v2.WITHDRAW),
  )
  console.log(
    'AAVE_WITHDRAW_ACTION',
    AAVE_WITHDRAW_ACTION,
    SERVICE_REGISTRY_NAMES.aave.v2.WITHDRAW,
    aaveWithdrawAction,
  )

  const aavePaybackAction = utils.keccak256(
    utils.toUtf8Bytes(SERVICE_REGISTRY_NAMES.aave.v2.PAYBACK),
  )
  console.log(
    'AAVE_PAYBACK_ACTION',
    AAVE_PAYBACK_ACTION,
    SERVICE_REGISTRY_NAMES.aave.v2.PAYBACK,
    aavePaybackAction,
  )
})
